...
enum : uint { N = 10000000, MODULUS_445 = 1000000007 };
...
void verify_factorization(uint n, primefac *pf)
{
	uint prod = 0;
	if (n > 0) {
		prod = 1;
	...
	if (prod != n) {
		printf("init_factor_table() failed on %d\n", n);
		exit(0);
	}
}
...
void init_factor_table()
{
	...
	for (uint i = 0; i <= N; ++i)
	{
		...
		if (i % 100000 == 0) {
			//printf("init factor tables: %.3f%% complete\n", 100.0*i / N);
		}
		...
		memcpy(factorized[i] = (primefac*)malloc(bytes), scratch, bytes);

		//verify_factorization(i, factorized[i]);
	}
}
...
uint euler445()
{
	...
	for (uint i = N, j = 1; i >= j; --i, ++j)
	{
		...
		union_factorlists(i_over_j, factorized[i], factorized[j], 1, -1);
